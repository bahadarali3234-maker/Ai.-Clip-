...
import { JobResult, JobStatusResponse } from "./types";
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      console.error("Failed to load local history state:", e);
    }
  }, []);

  // Poll job status background engine
  const startPolling = (jobToPoll: string) => {
    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Unable to contact backend extraction server.");
        }
        ...
        if (data.status === "completed" && data.result) {
          ...
          // Save completed clips to user history
  ...
  const addToHistory = (id: string, clipResult: JobResult) => {
    setSavedClips((prev) => {
      ...
      const updated = [newClip, ...prev].slice(0, 8); // Keep up to 8 clips in timeline
      try {
  ...
  const handleSelectHistory = (clip: SavedClip) => {
    // Clear active processes, directly pull cached result
    if (pollingRef.current) clearInterval(pollingRef.current);
  ...
  return (
    <div className="min-h-screen bg-[#050508] relative overflow-x-hidden text-gray-100 flex flex-col justify-between" id="clipai-app-root">
      ...
      {/* Main Container */}
      <div className="w-full max-w-7xl mx-auto px-4 py-8 md:py-12 space-y-12 relative z-10 flex-1">
        
        {/* Header Hero Area */}
        <header className="text-center space-y-4 max-w-4xl mx-auto" id="applet-intro-header">
          <motion.div
          ...
          >
            Clip<span className="bg-clip-text text-transparent bg-gradient-to-r from-violet-400 to-indigo-400 drop-shadow-[0_0_30px_rgba(124,58,237,0.3)]">AI</span>
          </motion.h1>
        ...
        <main className="space-y-10">
          ...
          <div className="space-y-10">
            ...
            {/* Completed Output preview section */}
            <AnimatePresence>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-xs text-rose-500/80 hover:text-rose-400 flex items-center gap-1.5 px-2.5 py-1 rounded bg-rose-500/5 hover:bg-rose-500/10 cursor-pointer border border-rose-500/10 transition-colors font-mono"
                  id="clear-clip-history"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4" id="saved-clips-grid">
                {savedClips.map((clip) => (
                  <button
                    key={clip.jobId}
      ...
      {/* Humble Footer */}
      <footer className="w-full text-center py-6 border-t border-white/5 bg-[#030306]/90 relative z-10" id="clipai-dashboard-footer">
